...
/*!
 * Store the period time for task thetask
 */
  __po_hi_milliseconds (&(period), 1000);
  
/*!
 * \brief Making Periodic Task thetask
 *
 * Make a periodic task according to AADL model requirements. The first 
 * parameter is the task identifier defined in deployment.h (pr_a_thetask_k)
 *  the second is the period defined in the AADL model. Third is the task 
 * priority ( 0), fourth is the stack size ( 0 bytes) and last is the 
 * subprogram executed by the task
 */
  __po_hi_create_periodic_task (pr_a_thetask_k, &(period), __PO_HI_DEFAULT_PRIORITY, 0, 0, thetask_job);
